...
vectime=linspace(0,0,max);
    ...
    %% MEX Function
        ...
        if i == 2500
            rXtraj=linspace(rX,rX,m*5);    % Trajectory x-points                 ((1xm) vector)
            rYtraj= linspace(rY,rY,m*5); 
            vel=0
            vTraj=linspace(vel,vel,m*5);     % Trajectory velocity                 ((1xm) vector)
        end
        ...
        vec_calc_v(i)=calc_v;
        %pause(0.01)
        tTraj=tTraj+0.01;  % Trajectory time_stamp               ((1xm) vector)
        rXtraj=rX+Xtraj;
        rYtraj=rY+Ytraj;
