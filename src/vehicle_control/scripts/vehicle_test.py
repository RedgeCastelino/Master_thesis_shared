...
def vehicle():
    ...
    Yaw=rospy.get_param("Veh_yaw")
    #print (ego_data)
    ego_data.object.orientation.yaw = Yaw*math.pi/180
...
def callback(traj):
    ...
    print("time step  ", time_step, "        calculated velocity", safe_div((res[0] - old_pos), time_step), "velocity",
          res[2])  # , "posx ", res[0], "     posy ",res[1], "   yaw  ",res[5], "     velocity x",res[2])
    # print (res)
