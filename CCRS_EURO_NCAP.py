...
"""
   EURO NCAP Test Scenario - Car to Pedestrian Nearside Child
"""
...
class World(object):
    ...
    def restart(self):
        ...
        ##[0] = Audi a2
        ##[10] = BMW Grandtour
        ##[15] = Bicicle Diamondback
        ##[23] = Seat Leon
        ##[25] = Nissan Patrol


        blueprintPlayer = blueprint[0]
        blueprintNpc1 = blueprint[23]
        ...
        while self.player is None:
            ...
            self.player.set_velocity(carla.Vector3D(x=0.0000, y=0.000000, z=0.000000))
       
            # Spawn the npc cars
            spawn_point = carla.Transform(carla.Location(x=219.5872746, y=12.62206999, z=1.843102), carla.Rotation(pitch=0.000000, yaw=0.855823, roll=0.000000))
            ...
            spawn_point = carla.Transform(carla.Location(x=214.2239483, y=12.54190365, z=1.843102), carla.Rotation(pitch=0.000000, yaw=0.855823, roll=0.000000))
            ...
            blueprintsWalkers = blueprintsWalkers[7]
            if blueprintsWalkers.has_attribute('is_invincible'):
                blueprintsWalkers.set_attribute('is_invincible', 'false')
            spawn_point = carla.Transform(carla.Location(x=223.013613, y=13.82338109, z=2.063102), carla.Rotation(pitch=0.000000, yaw=270.855823, roll=0.000000))
...
class CameraManager(object):
    def __init__(self, parent_actor, hud):
        ...
        self._camera_transforms = [carla.Transform(carla.Location(x=3.0, z=1.5), carla.Rotation(pitch=0)),carla.Transform(carla.Location(x=1.6, z=1.7))]
...
def game_loop(args):
    ...
    try:
        ...
        while True:
            ...
            # if position of the vehicle is higher than: Position of the walker - desired velocity of the ego * distance of the walker and the colision point/ velocity of the walker *cos of the street inclination
            #if player_pos.x > (walker_pos.x-player_size-walker_size-(args.velocity*4*math.cos(0.014937))/5):
            empiric_delay = 0.075 ## empiric parameter in a way that the pedestrian collide in the middle of the car 'system delay'
            if player_pos.x > ((walker_pos.x-player_size-walker_size-args.velocity*4/5-args.velocity*empiric_delay)*math.cos(0.014937)):
...
def main():
    argparser = argparse.ArgumentParser(
        description='CARLA AEB Scenario NCAP')
    argparser.add_argument('-v', '--verbose',
        action='store_true',
        dest='debug',
        help='print debug information')
    argparser.add_argument('-vel', '--velocity',
        metavar='V',
        default=40,
        type = float,
        help='Desired velocity of the EGO em Km/h (default: 40 Km/h)')
